...
struct PongIntroScene : public Scene
{
  ...
  void init()
  {
    ...
    gameCanvas.selectFont(&fabgl::FONT_10x20);
    gameCanvas.setPenColor(217, 245, 255);
    gameCanvas.setGlyphOptions(GlyphOptions().DoubleWidth(1));
    gameCanvas.drawText(centerText("PONG", 20), 20, "PONG");

    gameCanvas.selectFont(&fabgl::FONT_8x8);
    gameCanvas.setGlyphOptions(GlyphOptions().DoubleWidth(0));
    gameCanvas.setPenColor(224, 158, 16);
    gameCanvas.drawText(centerText("con ESP32 por FIE", 8), 65, "con ESP32 por FIE");
    gameCanvas.drawText(centerText("Facultad de Ingenieria Electrica.", 8), 80, "Facultad de Ingenieria Electrica.");

    gameCanvas.drawBitmap(PADDLE1_START_X, PADDLE_START_Y - 20, &bmpPaddle);
    gameCanvas.drawBitmap(PADDLE2_START_X, PADDLE_START_Y + 15, &bmpPaddle);

    gameCanvas.drawBitmap(BALL_START_X - 60, BALL_START_Y + 55, &bmpBall_2);
    gameCanvas.drawBitmap(BALL_START_X - 58, BALL_START_Y + 57, &bmpBall_1);
    gameCanvas.drawBitmap(BALL_START_X - 56, BALL_START_Y + 59, &bmpBall);

    gameCanvas.setPenColor(255, 255, 255);
    gameCanvas.drawRectangle(17, 4, 301, 5);
    gameCanvas.drawRectangle(17, 188, 301, 189);
  ...
  void update(int updateCount)
  {
    ...
    else
    {
      if (updateCount % 20 == 0)
      {
        gameCanvas.setPenColor(255, random(255), random(255));
        gameCanvas.drawText(centerText("Presiona [START] para jugar", 8), 100, "Presiona [START] para jugar");
      }
  ...
  int centerText(const char* text, int charLength){
    return (State::display.getViewPortWidth() - strlen(text) * charLength)/2;
  }
...
struct PongGameScene : public Scene
{
  ...
  void init()
  {
    ...
    gameCanvas.drawRectangle(17, 188, 301, 189);
    for(int i = 1; i <= 46; i++){
      gameCanvas.drawRectangle(POINT_START_X, POINT_START_Y, POINT_END_X, POINT_END_Y);
      POINT_START_Y += 4;
      POINT_END_Y += 4;
    }
    gameCanvas.setGlyphOptions(GlyphOptions().FillBackground(true));
    gameCanvas.selectFont(&fabgl::FONT_8x16);
  }
  ...
  {
    gameCanvas.setPenColor(255, 255, 255);
    gameCanvas.drawTextFmt(130, 10, "%02d" , scoreP2_);
    gameCanvas.drawTextFmt(172, 10, "%02d" ,scoreP1_);
  }
...
{
  Serial.println("Pong is executing");
  PongIntroScene introScene;
  introScene.start();
  PongGameScene gameScene;
  gameScene.start();
}
}